...
app.post(`${url}`, async (req, res) => {
    ...
    let sql = await database.execute(`INSERT INTO ${table}
                                        (nome, senha, token, email)
                                    VALUES
                                        ('${corpo.nome}', '${corpo.senha}', '${corpo.token}',  '${corpo.email}');`)
...
app.patch(`${url}/:id`, async (req, res) => {
    ...
    await database.execute(`UPDATE ${table} SET
                                ...
                                senha='${dados.senha || jaExiste[0].senha}',
                                token='${dados.token || jaExiste[0].token}',
                                email='${dados.email || jaExiste[0].email}'
                            WHERE id=${req.params.id};
                            `);
